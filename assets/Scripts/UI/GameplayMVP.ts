import { _decorator, BlockInputEvents, Button, Color, Component, director, EventTouch, game, Game, Graphics, Label, Mask, Node, NodePool, ResolutionPolicy, resources, screen, Sprite, SpriteFrame, tween, Tween, UIOpacity, UITransform, Vec2, Vec3, view } from 'cc';
...
import { ModeQuestionDirector } from '../domain/ModeQuestionDirector';
import { QuestionCompiler, type QuestionRequest } from '../domain/QuestionSystem';
...
import { showGameplayPauseOverlay } from './GameplayPauseOverlay';
...
export class GameplayMVP extends Component {
    private session!:GameSession; private generator!:QuestionCompiler; private director!:ModeQuestionDirector|TowerDirector; private visual!:SeededRng;
    ...
    private paused=false; private hidden=false; private hitStopActive=false; private manualPaused=false; private targetRevealPending=false; private revealToken=0; private resumeGeneration=0; private lastCountdownSecond=-1;
    private pauseButton:Node|null=null; private pauseOverlay:Node|null=null;
    private currentDirective:QuestionRequest|null=null;
    ...
    protected onLoad():void {
        ...
        const historyOptions={recentFactIds:saved.recentQuestionIds,recentSemanticSignatures:saved.recentQuestionSignatures,onQuestionAccepted:(ids:readonly string[],signature:string)=>AppRuntime.save.rememberQuestion(ids,signature)};this.generator=new QuestionCompiler(new SeededRng(`${AppRuntime.entry.seed}:gameplay`),GAMEPLAY_CONFIG,AppRuntime.entry,historyOptions);this.director=AppRuntime.entry.mode==='tower'?new TowerDirector(new SeededRng(`${AppRuntime.entry.seed}:director`),AppRuntime.entry.towerFloor??saved.tower.currentFloor):new ModeQuestionDirector(new SeededRng(`${AppRuntime.entry.seed}:director`),AppRuntime.entry,AppRuntime.entry.mode==='brawl60'?allowedBrawlRules(saved.tower,saved.tutorials):undefined,AppRuntime.entry.mode!=='brawl60'||saved.tower.highestClearedFloor>=15);this.visual=new SeededRng(`${AppRuntime.entry.seed}:visual`);
        ...
        this.bindStaticView();this.prepareTowerReadyCard();const endless=this.session.entry.mode==='brawl60';applyGameFontToTree(this.node,label=>!endless||label!==this.timer);if(endless){this.timer.font=null;this.timer.useSystemFont=true;this.timer.fontFamily='Arial';}screen.on('window-resize',this.handleResize,this);game.on(Game.EVENT_HIDE,this.onHide,this);game.on(Game.EVENT_SHOW,this.onShow,this);this.scheduleOnce(this.handleResize,0);
    ...
    private buildHandDrawnChrome():void{
        ...
        this.timer?.node.setSiblingIndex(this.node.children.length-1);
    }
    ...
    private makePauseButton(parent:Node):void{
        const button=node('PauseButton',parent,72,72);button.active=false;button.addComponent(BlockInputEvents);const artwork=image(button,'Artwork',72,72);artwork.trim=false;
        resources.load('textures/gameplay/ui/pause/spriteFrame',SpriteFrame,(e,f)=>{if(!e&&f?.isValid&&artwork.isValid)artwork.spriteFrame=f;});
        const control=button.addComponent(Button);control.transition=Button.Transition.SCALE;control.zoomScale=.94;button.on(Button.EventType.CLICK,this.showPauseMenu,this);this.pauseButton=button;
    }
    private layoutHandDrawnChrome(width:number,height:number):void{
        ...
        chrome.getChildByName('PauseButton')?.setPosition(-width/2+58,frameTop-40);
    ...
    private spawn():void {
        ...
        if(this.towerChallenge){this.towerRequest=this.towerChallenge.nextRequest();this.currentDirective=(this.director as TowerDirector).next(this.session.state.elapsedMs,this.towerRequest);}else this.currentDirective=this.director.next(this.session.state.elapsedMs);this.question=this.generator.next(this.currentDirective,this.session.entry.contentVersion);this.presentCurrentQuestion();
    }
    ...
    private refresh():void{if(!this.score)return;const s=this.session.state,endless=this.session.entry.mode==='brawl60',seconds=Math.ceil(s.remainingMs/1000);this.score.string=String(s.score);this.combo.string=String(s.combo);this.timer.string=endless?'∞':`${seconds}s`;if(this.pauseButton)this.pauseButton.active=(s.phase==='playing'||s.phase==='resolving')&&!this.manualPaused&&!this.hidden&&!this.finished;this.updateLifeHearts(s.life);this.updateFriendTarget(s.score);this.updateDailyTarget(s.score);this.updateTowerProgress(s.correctCount);if(!endless)this.updateCountdownFeedback(s.remainingMs);}
    ...
    private applyHitStop(durationMs:number):void{this.hitStopActive=true;this.syncPausedState();this.scheduleOnce(()=>{this.hitStopActive=false;this.syncPausedState();},durationMs/1000);}
    ...
    private showPauseMenu():void{
        if(this.finished||this.hidden||this.manualPaused||this.session.state.phase!=='playing')return;
        this.manualPaused=true;this.resumeGeneration++;this.syncPausedState();this.cancelActiveTouch();if(this.pauseButton)this.pauseButton.active=false;
        this.pauseOverlay=showGameplayPauseOverlay(this.node,{continueGame:()=>this.continueFromPause(),home:()=>this.homeFromPause()});
        AppRuntime.audio.play('ui');director.pause();
    }
    private continueFromPause():void{
        if(!this.manualPaused)return;AppRuntime.audio.play('ui');this.manualPaused=false;this.pauseOverlay?.destroy();this.pauseOverlay=null;director.resume();this.beginResumeCountdown();
    }
    private homeFromPause():void{
        if(!this.manualPaused)return;AppRuntime.audio.play('ui');this.manualPaused=false;this.pauseOverlay?.destroy();this.pauseOverlay=null;this.finished=true;this.paused=true;this.revealToken++;director.resume();AppRuntime.home();
    }
    private beginResumeCountdown():void{
        const token=++this.resumeGeneration;this.paused=true;this.node.getChildByName('ResumeReady')?.destroy();const ready=text(this.node,'ResumeReady','READY',68,RED);
        this.scheduleOnce(()=>{if(token!==this.resumeGeneration)return;ready.node.destroy();this.syncPausedState();this.refresh();},GAMEPLAY_CONFIG.readyMs/1000);
    }
    private cancelActiveTouch():void{this.pendingTouchPoint=null;this.lastQueuedTouchPoint=null;this.touchActive=false;this.points=[];if(!this.keepsIncompleteGesture())this.gesture=null;if(this.trailVisible){this.trail.clear();this.trailVisible=false;}}
    private syncPausedState():void{this.paused=this.hidden||this.manualPaused||this.hitStopActive;}
    private onHide():void{if(this.finished)return;this.hidden=true;this.resumeGeneration++;this.syncPausedState();this.cancelActiveTouch();director.pause();}
    private onShow():void{if(this.finished)return;this.hidden=false;this.syncPausedState();if(this.manualPaused)return;director.resume();this.beginResumeCountdown();}
}
