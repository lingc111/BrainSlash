...
const TARGET_SHAPES: readonly TargetShape[] = ['roundedSquare','hexagon','circle','pentagon'];
...
export class GameplayMVP extends Component {
    ...
    private success(t:GameplayTarget|null):void{if(!this.question)return;const r=this.session.resolveSuccess(this.question);if(!r)return;const pos=t?.node.position??Vec3.ZERO,feedback=successFeedback(r.kind,this.session.state.combo),vibration=AppRuntime.save.snapshot().settings.vibration;AppRuntime.audio.play(feedback.sound,{variant:this.session.state.combo});AppRuntime.platform.vibrate(vibration,feedback.haptic);if(feedback.comboMilestone&&r.kind!=='master')AppRuntime.audio.play('combo',{variant:this.session.state.combo});this.animateCombo(this.session.state.combo,feedback.comboMilestone);this.hitSparks(pos,r.kind==='master'?YELLOW:GREEN,r.kind==='master');if(feedback.hitStopMs)this.applyHitStop(feedback.hitStopMs);this.float(pos,`+${r.scoreDelta}${r.kind==='master'?' MASTER':''}`,r.kind==='master'?YELLOW:GREEN);if(r.lifeDelta>0){AppRuntime.audio.play('combo',{variant:5});AppRuntime.platform.vibrate(vibration,'medium');this.float(new Vec3(pos.x,pos.y+70,pos.z),'♥ +1 生命',RED);}const towerFloor=this.session.entry.towerFloor??1,towerCleared=this.session.entry.mode==='tower'&&this.session.state.correctCount>=towerFloorConfig(towerFloor).requiredCorrect;if(towerCleared){this.session.finish();return;}this.scheduleOnce(()=>{this.session.continueAfterFeedback();this.spawn();},r.kind==='master'?.32:.28);}
    private fail(kind:FailureKind,t:GameplayTarget|null=null):void{const pos=t?.node.position??Vec3.ZERO,vibration=AppRuntime.save.snapshot().settings.vibration;const brokenCombo=this.session.state.combo;if(!this.session.resolveFailure(kind))return;if(this.question&&this.constraint)this.mistakes.push(createMistakeRecord(this.question,this.constraint,kind,t?.data.id));const feedback=failureFeedback(kind,brokenCombo);AppRuntime.audio.play(feedback.sound);AppRuntime.platform.vibrate(vibration,feedback.haptic);if(kind==='wrong'||kind==='miss')this.showWrongAnswer(pos);else{this.error(pos);if(feedback.label)this.float(pos,feedback.label,RED);}if(feedback.showComboBreak)this.breakCombo(brokenCombo);if(this.session.state.phase!=='finished')this.scheduleOnce(()=>{this.session.continueAfterFeedback();this.spawn();},.3);}
