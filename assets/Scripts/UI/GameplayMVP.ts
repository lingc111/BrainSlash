...
export class GameplayMVP extends Component {
    ...
    private finish():void{if(this.finished)return;this.finished=true;AppRuntime.audio.play('finish');const s=this.session.state,total=s.correctCount+s.errorCount;const run:RunResult={entry:this.session.entry,score:s.score,maxCombo:s.maxCombo,correctCount:s.correctCount,errorCount:s.errorCount,accuracy:total?s.correctCount/total:0,bestReactionMs:s.bestReactionMs,remainingMs:s.remainingMs,elapsedMs:s.elapsedMs,masterSlashCount:s.masterSlashCount,mistakes:[...this.mistakes]};if(this.session.entry.mode==='tower'){const result=AppRuntime.finishTower(run,s.life);showTowerResultOverlay(this.node,result,{next:()=>AppRuntime.nextTowerFloor(),retry:()=>AppRuntime.retryTowerFloor(),home:()=>AppRuntime.home()});return;}const result=AppRuntime.finish(run);showResultOverlay(this.node,result,{replay:()=>AppRuntime.replay(),share:()=>AppRuntime.share(),home:()=>AppRuntime.home()});}
