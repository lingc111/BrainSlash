...
const SKINS = ['blue_square','green_triangle','orange_circle','pink_diamond','purple_hexagon','red_trapezoid','yellow_circle'] as const;
...
export class GameplayMVP extends Component {
    ...
    private bindStaticView():void {
        ...
        this.effects=required('HitEffects');this.floats=required('FloatingText');this.score=requiredLabel('Score');this.combo=requiredLabel('Combo');this.prompt=requiredLabel('Prompt');this.rule=requiredLabel('Rule');this.prepareRuleBadge();this.timer=requiredLabel('Timer');this.life=requiredLabel('Life');
    ...
    private buildHandDrawnChrome():void{
        ...
        const dailyCard=node('DailyChallengeProgress',chrome,550,48),dailyBadge=dailyCard.addComponent(Graphics);dailyBadge.fillColor=new Color(255,250,236,245);dailyBadge.strokeColor=BLUE;dailyBadge.lineWidth=3;dailyBadge.roundRect(-275,-24,550,48,16);dailyBadge.fill();dailyBadge.stroke();const dailyLabel=text(dailyCard,'Label','',21,INK);ui(dailyLabel.node,525,44);dailyLabel.overflow=Label.Overflow.SHRINK;dailyCard.active=this.session?.entry.mode==='daily';this.dailyTarget=dailyLabel;this.dailyTargetCard=dailyCard;
    }
    ...
    private layoutHandDrawnChrome(width:number,height:number):void{
        ...
        chrome.getChildByName('DailyChallengeProgress')?.setPosition(0,hudY-100);
    ...
    private success(t:GameplayTarget|null):void{if(!this.question)return;const r=this.session.resolveSuccess(this.question);if(!r)return;const pos=t?.node.position??Vec3.ZERO,feedback=successFeedback(r.kind,this.session.state.combo),vibration=AppRuntime.save.snapshot().settings.vibration;AppRuntime.audio.play(feedback.sound,{variant:this.session.state.combo});AppRuntime.platform.vibrate(vibration,feedback.haptic);if(feedback.comboMilestone&&r.kind!=='master')AppRuntime.audio.play('combo',{variant:this.session.state.combo});this.animateCombo(this.session.state.combo,feedback.comboMilestone);this.hitSparks(pos,r.kind==='master'?YELLOW:GREEN,r.kind==='master');if(feedback.hitStopMs)this.applyHitStop(feedback.hitStopMs);this.float(pos,`+${r.scoreDelta}${r.kind==='master'?' MASTER':''}`,r.kind==='master'?YELLOW:GREEN);const towerFloor=this.session.entry.towerFloor??1,towerCleared=this.session.entry.mode==='tower'&&this.session.state.correctCount>=towerFloorConfig(towerFloor).requiredCorrect;if(towerCleared){this.session.finish();return;}this.scheduleOnce(()=>{this.session.continueAfterFeedback();this.spawn();},r.kind==='master'?.32:.28);}
    ...
    private finish():void{if(this.finished)return;this.finished=true;AppRuntime.audio.play('finish');const s=this.session.state,total=s.correctCount+s.errorCount;const run:RunResult={entry:this.session.entry,score:s.score,maxCombo:s.maxCombo,correctCount:s.correctCount,errorCount:s.errorCount,accuracy:total?s.correctCount/total:0,bestReactionMs:s.bestReactionMs,remainingMs:s.remainingMs,mistakes:[...this.mistakes]};if(this.session.entry.mode==='tower'){const result=AppRuntime.finishTower(run,s.life);showTowerResultOverlay(this.node,result,{next:()=>AppRuntime.nextTowerFloor(),retry:()=>AppRuntime.retryTowerFloor(),home:()=>AppRuntime.home()});return;}const result=AppRuntime.finish(run);showResultOverlay(this.node,result,{replay:()=>AppRuntime.replay(),share:()=>AppRuntime.share(),home:()=>AppRuntime.home()});}
    private refresh():void{if(!this.score)return;const s=this.session.state,seconds=Math.ceil(s.remainingMs/1000);this.score.string=String(s.score);this.combo.string=String(s.combo);this.timer.string=`${seconds}s`;this.updateLifeHearts(s.life);this.updateFriendTarget(s.score);this.updateDailyTarget(s.score);this.updateTowerProgress(s.correctCount);this.updateCountdownFeedback(s.remainingMs);}
