...
export class GameSession {
    ...
    public beginQuestion(): void { if (this.state.phase === 'playing') { this.questionStartedAt = this.state.elapsedMs; this.masterWindowStartedAt = this.state.elapsedMs + this.config.masterHitSettleDelayMs; this.questionResolved = false; } }
    ...
    public resolveSuccess(question: QuestionInstance, masterSlash = false): Extract<HitResult, { kind: 'correct' | 'master' | 'masterSlash' }> | null {
        if (this.questionResolved || this.state.phase !== 'playing') return null;
        this.questionResolved = true;
        const reactionMs = this.questionElapsedMs(), masterReactionMs = Math.max(0, this.state.elapsedMs - this.masterWindowStartedAt), master = masterReactionMs <= this.config.masterWindowMs;
        const scoreDelta = calculateScore(this.config, this.state.combo, question.activeRules.filter((rule) => rule !== 'standard').length, master, masterSlash);
