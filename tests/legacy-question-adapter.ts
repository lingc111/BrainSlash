...
import type { ContentFamilySpec } from '../assets/Scripts/domain/ContentCatalog.ts';
...
import { targetCountForFamily, type TemplateCompileDirective } from '../assets/Scripts/domain/QuestionPolicy.ts';
...
/** Test-only bridge for legacy directive-shaped unit fixtures. Production cannot import this module. */
export class DirectiveCompilerFixture {
    ...
    public next(directive: TemplateCompileDirective): QuestionInstance {
        return this.compiler.next({
            templateIds: [directive.family.kind], themes: [directive.family.theme], rules: directive.rules,
...
class LegacyModeDirector {
    ...
    public next(elapsedMs: number): TemplateCompileDirective {
        ...
        return { phase: request.phase, difficultyStage: request.difficulty <= 1 ? 0 : request.difficulty <= 3 ? 1 : 2,
            targetCount: targetCountForFamily(request.targetCount, family.kind, request.rules), questionTimeMs: request.questionTimeMs,
            speed: request.speed, family, rules: [...request.rules], bombEnabled: request.rules.includes('bomb') };
    }
}

export class BrawlRequestFixture extends LegacyModeDirector {
...
export class FriendChallengeRequestFixture extends LegacyModeDirector {
